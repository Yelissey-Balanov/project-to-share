<template>
  <tr class="group">
    <td class="px-6 py-3.5">{{ revenue.month | wordForMonth }} {{ revenue.year }}</td>
    <td class="px-6 py-3.5">{{ (revenue.amount / 100) | currency }}</td>
    <td class="px-6 py-3.5">
      <span class="tbadge">{{ revenue.type | replaceUnderscoreWithSpace }}</span>
    </td>
    <td class="px-6 py-3.5">
    ...
    <td class="px-6 py-3.5">
      <div class="flex items-center">
        <div class="small-foto mr-4">
          <LImg
            :image="revenue.project.company.logo ? revenue.project.company.logo.sizes.PROFILE_IMAGE : null"
            v-if="revenue.project.company"
          >
            <IHOfficeBuilding class="w-8 h-8 text-gray-400" />
          </LImg>
        </div>
        <div class="flex flex-col">
          <nuxt-link :to="`/projects/${revenue.project.id}`">
            {{ revenue.project.title }}
          </nuxt-link>
          <nuxt-link :to="`/companies/${revenue.project.company.id}`" class="text-gray-500 hover:underline">
            {{ revenue.project.company.aliasOrName }}
          </nuxt-link>
        </div>
      </div>
    </td>
    <td class="px-6 py-3.5">
      <div class="flex space-x-2 items-center">
        <div class="flex flex-col" v-for="revenue_user in revenue.revenue_users" :key="revenue_user.id">
          <nuxt-link :to="`/users/${revenue_user.user.id}`" class="small-foto">
            <LImg
...
import { DeleteRevenueMutationVariables, GetRevenuesQuery, RevenueFragment, RevenueStage } from '~/graphql/GQLTypes'
...
@Component({
  components: { IHOfficeBuilding, IHEye, IHUserFilled, IHDelete, IHPencil },
})
export default class RevenuesOverviewTableRow extends Vue {
  @Prop()
  revenue!: NonNullable<GetRevenuesQuery['revenues']>['data'][0]
